...
// Access Recharts components if available
const { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, PieChart, Pie, Cell, BarChart, Bar } = window.Recharts || {};
...
;
      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          ...
          <button className="md:hidden p-2 text-gray-600 hover:bg-gray-100 rounded-full" onClick={() => setShowMobileMenu(!showMobileMenu)}>
            {showMobileMenu ? (X && <X className="w-5 h-5" />) : (Menu && <Menu className="w-5 h-5" />)}
          </button>
        </div>
      </div>
        ...
        <div className="max-h-96 overflow-y-auto">
          {notifications.length === 0 ? (
            ...
            notifications.map(notif => (
              ...
              >
                {notif.type === 'success' ? (
                  CheckCircle && <CheckCircle className="w-5 h-5 text-green-500 mr-3 flex-shrink-0" />
                ) : notif.type === 'error' ? (
                  XCircle && <XCircle className="w-5 h-5 text-red-500 mr-3 flex-shrink-0" />
                ) : (
                  AlertCircle && <AlertCircle className="w-5 h-5 text-blue-500 mr-3 flex-shrink-0" />
                )}
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center space-x-2">
              {opportunity.profitPercentage > 0 ? (
                ArrowUpRight && <ArrowUpRight className="w-5 h-5 text-green-500" />
              ) : (
                ArrowDownRight && <ArrowDownRight className="w-5 h-5 text-red-500" />
              )}
            </div>
          </div>
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        ...
        >
          {isLoading ? (
            ...
            <>
              {Search && <Search className="w-5 h-5 mr-2" />}
              Find Opportunities
            </>
          )}
        </button>
  ...
  const DashboardView = () => (
    <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              {Search && <Search className="w-6 h-6 text-green-500" />}
            </div>
      ...
      {stats && stats.top_categories && BarChart && (
