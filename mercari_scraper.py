...
class MercariScraper:
    """Enhanced Mercari scraper to find product listings"""
    ...
    async def search_listings(self, keywords: List[str], max_pages: int = 2) -> List[Dict]:
        ...
        for keyword in keywords:
            try:
                ...
                logger.info(f"Found {len(keyword_listings)} Mercari listings for keyword: {keyword}")
            ...
            try:
                ...
                async with self.session.get(url, headers=self.headers) as response:
                    if response.status != 200:
                        logger.warning(f"Mercari returned status code {response.status} for keyword '{keyword}', page {page}")
                        continue
    ...
    def _parse_mercari_search_results(self, html_content: str, keyword: str) -> List[Dict]:
        ...
        product_containers = soup.select('div[data-testid="ItemCell"]')
        ...
        for container in product_containers:
            try:
                ...
                title_element = container.select_one('[data-testid="itemName"]')
                title = title_element.text.strip() if title_element else "Unknown Title"
                
                # Extract price
                price_element = container.select_one('p[data-testid="itemPrice"]')
                ...
                # Check if it's sold
                sold_element = container.select_one('div[data-testid="itemStatus"]')
                is_sold = "sold" in sold_element.text.lower() if sold_element else False
                
                # Get shipping info if available
                shipping_element = container.select_one('p[data-testid="itemShippingLabel"]')
                shipping_info = shipping_element.text.strip() if shipping_element else ""
    ...
    async def get_product_details(self, listing_id: str) -> Dict:
        ...
        try:
            ...
            async with self.session.get(url, headers=self.headers) as response:
                if response.status != 200:
                    logger.warning(f"Mercari returned status code {response.status} for listing ID {listing_id}")
                    return {}
